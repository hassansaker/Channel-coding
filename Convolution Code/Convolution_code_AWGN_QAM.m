...
numBits = 1000*log2(M);                % Number of input bits
EbN0 = 0:0.5:9;                        % Signal-to-noise ratio range (dB)
SNR = EbN0 + 10*log10(log2(M));        % Convert SNR to Eb/N0 for M-QAM
...
numErrors_uncoded=0;
    ...
    % M-QAM Modulation (with bit input & unit average power)
    ...
    %-----------------Receiver------------------------
    % QAM Demodulation (output as bits)
    rxDataHard_uncoded = qamdemod(rxSig_uncoded, M, 'OutputType', 'bit', 'UnitAveragePower', true);
    ...
    % M-QAM Modulation (with bit input & unit average power)
    txSig_coded = qammod(encodedBits, M, 'InputType', 'bit', 'UnitAveragePower', true);

    %----------------Channel--------------------------
    % Add AWGN noise
    rxSig_coded = awgn(txSig_coded,SNR(i)+10*log10(1/2),'measured');
    
    %-----------------Receiver------------------------
    % Hard-Decision QAM Demodulation (output as bits)
    rxDataHard_coded = qamdemod(rxSig_coded, M, 'OutputType', 'bit', 'UnitAveragePower', true);
...
xlabel('Eb/N0 (dB)');
ylabel('BER');
title('BER Performance: Coded vs Uncoded M-QAM in AWGN Channel');
